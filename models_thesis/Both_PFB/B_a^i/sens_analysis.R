...
setwd("/shares/CIBIO-Storage/BCG/scratch2/tirocinanti/tonina/Both_PFB/B_a^i")
...
function_normal <- function(val,TF){
  system(paste0('../../../sim64 ../Cell-cycle_',TF,'_normal.prog ../../../Cell_cycle.types ../../../Cell_cycle.fun -o=',as.factor(val),'_B_a^i_results_',TF))
  
  upload <- fread(paste0(as.factor(val),'_B_a^i_results_',TF,'.E.out'), col.names =
  ...
  system(paste0('rm ',as.factor(val),'_B_a^i_results_',TF,'*'))
...
system(paste0('../../../sim64 B_a^i_Cell-cycle_',TF,'_',specie,'.prog ../../../Cell_cycle.types ../../../Cell_cycle.fun -o=',as.factor(iteration_x),'_B_a^i_results_',TF,'_',specie,'_',as.factor(value)))
  
  upload <- fread(paste0(as.factor(iteration_x),'_B_a^i_results_',TF,'_',specie,'_',as.factor(value),'.E.out'), col.names =
                    c('time','TR','TR_p','activator','activator_p','inhibitor','inhibitor_p','TF_a','TF_i','E_1',
                      'E_2','Chp_a','Chp_i','Pho','S','null'), data.table = F,sep = '\t',fill = T)
  
  
  system(paste0('rm ',as.factor(iteration_x),'_B_a^i_results_',TF,'_',specie,'_',as.factor(value),'*'))
  if (any(as.numeric(upload$TR_p) >= 150)){
...
for (value in seq(0,1000,50)){
    ...
    writeLines(bef1, con = paste0("B_a^i_Cell-cycle_500_inhibitor_p.prog"))

    bef50_1 <- gsub(pattern = "500 inhibitor_p", replace = word, x = bef50)
    writeLines(bef50_1, con = paste0("B_a^i_Cell-cycle_50_inhibitor_p.prog"))
...
for (value in seq(0,1000,50)) {
  ...
  writeLines(bef1, con = paste0("B_a^i_Cell-cycle_500_E1.prog"))

  bef50_1 <- gsub(pattern = "500 E_1", replace = word, x = bef50)
  writeLines(bef50_1, con = paste0("B_a^i_Cell-cycle_50_E1.prog"))
...
for(value in seq(0,1000,50)){
  ...
  writeLines(bef1, con = paste0("B_a^i_Cell-cycle_500_E2.prog"))

  bef50_1 <- gsub(pattern = "500 E_2", replace = word, x = bef50)
  writeLines(bef50_1, con = paste0("B_a^i_Cell-cycle_50_E2.prog"))
...
for (value in seq(0,1000,50)) {
  ...
  writeLines(bef1, con = paste0("B_a^i_Cell-cycle_500_TF_a.prog"))

  bef50_1 <- gsub(pattern = "50 TF_a", replace = word, x = bef50)
  writeLines(bef50_1, con = paste0("B_a^i_Cell-cycle_50_TF_a.prog"))
...
for (value in seq(0,1000,50)){
  ...
  writeLines(bef1, con = paste0("B_a^i_Cell-cycle_500_Chp_i.prog"))
  
  bef50_1 <- gsub(pattern = "500 Chp_i", replace = word, x = bef50)
  writeLines(bef50_1, con = paste0("B_a^i_Cell-cycle_50_Chp_i.prog"))
...
for (value in seq(0,1000,50)){
  ...
  writeLines(bef1, con = paste0("B_a^i_Cell-cycle_500_Pho.prog"))
  
  bef50_1 <- gsub(pattern = "500 Chp_i", replace = word, x = bef50)
  writeLines(bef50_1, con = paste0("B_a^i_Cell-cycle_50_Pho.prog"))
...
param_func <- function(iteration_x,val,parameter,TF){
  ...
  system(paste0('../../../sim64 ../Cell-cycle_',TF,'_normal.prog ../../../Cell_cycle.types B_a^i_Cell-cycle_',TF,'.fun -o=',as.factor(iteration_x),'_B_a^i_results_',TF,'_',parameter,'_',as.factor(val)))
  
  upload <- fread(paste0(as.factor(iteration_x),'_B_a^i_results_',TF,'_',parameter,'_',as.factor(val),'.E.out'), col.names =
                    c('time','TR','TR_p','activator','activator_p','inhibitor','inhibitor_p','TF_a','TF_i','E_1',
                      'E_2','Chp_a','Chp_i','Pho','S','null'), data.table = F,sep = '\t',fill = T)
  
  
  system(paste0('rm ',as.factor(iteration_x),'_B_a^i_results_',TF,'_',parameter,'_',as.factor(val),'*'))
  if (any(as.numeric(upload$TR_p) >= 150)){
...
for (par in file_fun[1:40]){
  ...
  if (value != '1/500' ){
    ...
    for (j in c(seq(0.1,1,0.1),seq(2,10,1))){
      ...
      writeLines(bef1, con=paste0("B_a^i_Cell-cycle_500.fun"))
      
      bef4  <- gsub(pattern = par, replace = word, x = file_fun)
      writeLines(bef4, con=paste0("B_a^i_Cell-cycle_50.fun"))
